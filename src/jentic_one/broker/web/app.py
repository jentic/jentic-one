...
def create_app(ctx: Context, container: AppContainer | None = None) -> FastAPI:
    ...
    async def broker_lifespan(app: FastAPI) -> AsyncGenerator[None]:
        ...
        # sync handler (via get_http_runner) and worker share.
        ...
        # In-flight gauge reads the shared admission gate every scrape.
        ...
        # the circuit breaker (memory default; Redis ⇒ cluster-wide).
        ...
        # Idempotency-Key replay store over the same shared backend. When
        ...
        # itself and hand it to build_runner for capability-gating.
        ...
        # Wrap the (optionally breaker-decorated) transport with the
        ...
        # The RunnerRegistry is the scheme→runner selection seam and
        ...
        # "One pipeline, two callers". The async worker (started by
        ...
        try:
            ...
            # Drain step 4 — close the shared client/runners and state
    ...
    # probe, so both observe the same in-flight counter.
