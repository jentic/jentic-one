...
class GovernedHostsResponse(BaseModel):
    ...
    **Hosts only, deliberately**: the set exists for interception scoping
    ...
    Deliberately **unpaginated**: the set is bounded by the caller's own
    credential bindings (tens of hosts, not thousands) and the digest must
    cover the whole set atomically — a paginated digest would be meaningless.
    This is a documented deviation from the list-endpoint pagination
    convention.
    """
    ...
    data: list[str] = Field(
        description=(
            ...
            "broker's discovery matches for the APIs the caller's bound "
            "credentials cover. "
            ...
            "(intercept-nothing) list."
        )
    )
    digest: str = Field(
        description=(
            "SHA-256 hex digest over the newline-joined `data` list; also emitted "
            "as the response's strong `ETag`. To change-poll, send it back "
            'quoted — `If-None-Match: "<digest>"` (the bare digest is accepted '
            "as a compatibility form) — and expect an empty `304` until the "
            "host set changes. Poll at most once per minute."
        )
    )
