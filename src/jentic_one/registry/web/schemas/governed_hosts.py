...
class GovernedHostsResponse(BaseModel):
    ...
    **Hosts only, deliberately**: the set exists for interception scoping
    ...
    data: list[str] = Field(
        description=(
            "Governed host patterns, lowercased, deduplicated, and sorted — the "
            "URL-index hosts the broker's discovery matches for the caller's "
            "toolkit-bound APIs. An entry may contain a `{var}` placeholder "
            "label (a defaultless server variable), which the broker matches as "
    ...
    digest: str = Field(
        description=(
            ...
            "as the response's strong `ETag` for If-None-Match change-polling."
        )
    )
