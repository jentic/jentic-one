"""Daemon-native MCP over Streamable HTTP — the ``/mcp`` mount.
...
surface — never the broker: the
broker stays MCP-free; ``execute`` proxies control-plane→broker server-side).

Tool surface: the pinned spec (``docs/reference/mcp-tools.json``, pinned from
