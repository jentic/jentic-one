...
| `mcp-tools.json` | The pinned MCP tool surface (names, descriptions, input schemas, annotations). The Go stdio server's `toolSpecs()` is the source of truth; `cli/internal/cli/api/mcp_spec_test.go` pins it here, the backend's `/mcp` mount loads it from the packaged copy, and `tests/unit/mcp/test_tool_surface.py` fails on any divergence. | `UPDATE_MCP_SPEC=1 go test ./internal/cli/api` (from `cli/`) |
