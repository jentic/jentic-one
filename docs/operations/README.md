...
One recurring human task lives in the UI rather than these pages: granting
an agent access. Access is default-deny — an operator stores a credential
and binds the agent to it on the agent's detail page in the console, or
consents to an agent-initiated vendor connect flow, which creates the
credential and the binding in one approved step. The flow end to end:
