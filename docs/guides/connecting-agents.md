...
4. **Raw HTTP** — for runtimes with none of the above, every deployment
   ...
   exchange, discovery, access checks (`GET /me`), and brokered execution.

If a session has both the CLI and MCP tools, prefer the MCP tools and use the
CLI for `setup` recovery and anything not exposed over MCP (a connect flow is
`request_connection` over MCP, `jentic connect` on the CLI) — both
