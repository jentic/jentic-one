...
> `docs/security/security.md`, and the code in this repo. It deliberately does **not**
...
Deployment personas the public docs call out (`docs/security/security.md`):
...
- **Not co-located with the agent for _real / high-value credentials_.** *(Grounded —
  ...
  (`SECURITY.md`, `docs/security/security.md`). **Nuance:** same-host use *is*
...
3. **Self-hostable & operable by a small team.** One-command install; tiered
   self-serve hardening path (`README.md`, `docs/security/security.md`).
