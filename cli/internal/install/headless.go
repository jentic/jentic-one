// headless.go — non-interactive install support (CLI-V2 Phase 9, impl/9.0 §9.2).
//
// The install wizard is a TTY-only Bubbletea TUI, which makes unattended
// installs (CI, scripted server provisioning, the cross-platform E2E matrix)
// impossible. `jenticctl install --defaults` takes the wizard's defaults as-is;
// `--answers <file>` overlays explicit answers from YAML on top of the defaults.
...
// Pointer fields distinguish "not provided" (keep the default) from an explicit
// zero value ("debug: false"). Field names are the wizard's own vocabulary,
// grouped as the hub sections present them.
type Answers struct {
...
// Apply overlays the provided answers onto the draft. Only fields present in the
// file are touched; everything else keeps the NewDraft default.
