...
const (
	...
	// legacyInitSchemasName is the bootstrap SQL older installs wrote beside
	// the compose file and mounted into the Postgres init directory. Schema
	// creation now happens idempotently inside the migration runner (#992 —
	// initdb scripts run once, on an empty data dir, so a mid-init failure
	// left a volume that silently never got its schemas). The name survives
	// only so install can clean the stale file up.
	legacyInitSchemasName = "init-schemas.sql"
)
...
// bootstrap SQL, so WriteComposeArtifacts can remove the stale file.
func (c ComposeConfig) legacyInitSchemasPath() string {
	return filepath.Join(filepath.Dir(c.ComposePath), legacyInitSchemasName)
}
...
type composeParams struct {
	...
	// BindHost prefixes every published port. Docker publishes unqualified
	// port mappings on ALL interfaces, so the wizard's bind-host answer must
	// be carried into the mappings or a loopback-intended install exposes its
	// app, broker, and database to the network (#992).
	BindHost        string
	ServerPort      string
	BrokerPort      string
	...
	// PGExposePort publishes the managed Postgres 5432 on the host. Off by
	// default (#992): the app/broker reach the db over the compose network,
	// so a host publish is a debugging convenience only.
	PGExposePort  bool
	DBService     string
...
func RenderCompose(d *Draft, cfg ComposeConfig) ([]byte, error) {
	// The managed Postgres must never come up passwordless: FillSecrets
	// generates a random credential precisely because the guessable default
	...
	params := composeParams{
		...
		PGExposePort:        d.PGExposeHostPort,
...
// WriteComposeArtifacts renders and writes the compose file and ensures the
...
// the host user) reads it, and it can carry database credentials.
func WriteComposeArtifacts(d *Draft, cfg ComposeConfig) error {
	...
	// Schema bootstrap moved into the migration runner (#992); remove the
