...
func newSkillsCmd(app *App) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return app.skillsList(cmd, opts)
			}
			return app.skillsShow(args[0])
		},
	}
	cmd.Flags().BoolVar(&opts.json, "json", false, "output as JSON array")

	cmd.AddCommand(newSkillsInitCmd(app))
	return cmd
}

func newSkillsInitCmd(app *App) *cobra.Command {
	opts := &skillOptions{}
	cmd := &cobra.Command{
		Use:   "init [name]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return app.skillsInit(cmd, opts, nameFilter)
		},
	}
...
func (a *App) skillsList(cmd *cobra.Command, opts *skillOptions) error {
...
func (a *App) skillsShow(name string) error {
...
func (a *App) skillsInit(_ *cobra.Command, opts *skillOptions, nameFilter string) error {
...
func (a *App) writeTaskSkills(ad skillgen.Adapter, env skillgen.DetectEnv, scope skillgen.Scope, skills []skillgen.TaskSkill, dryRun bool) (int, error) {
	switch ad.Operator() {
...
func (a *App) writeTaskSkillsDir(ad skillgen.Adapter, env skillgen.DetectEnv, scope skillgen.Scope, skills []skillgen.TaskSkill, dryRun bool) (int, error) {
	base := env.Home
...
func (a *App) writeTaskSkillsHermes(env skillgen.DetectEnv, scope skillgen.Scope, skills []skillgen.TaskSkill, dryRun bool) (int, error) {
	base := env.Home
	if scope == skillgen.ScopeProject {
		base = env.Cwd
	}
...
func (a *App) writeTaskSkillsAgents(ad skillgen.Adapter, env skillgen.DetectEnv, scope skillgen.Scope, skills []skillgen.TaskSkill, dryRun bool) (int, error) {
	...
	content := b.String()
	// Remove any existing "## Task Skills" section before appending the new one.
...
func removeTaskSkillsSection(s string) string {
	...
	// Find the next H2 after "## Task Skills" to preserve remaining content.
