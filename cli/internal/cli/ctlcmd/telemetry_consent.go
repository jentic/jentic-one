...
func (a *app) ensureTelemetryConsent(interactive bool) (proceed bool, enabled bool, err error) {
	...
	if interactive {
		...
		fmt.Fprintln(a.Out, theme.Dim.Render("family (linux/macOS/windows, anything else as \"other\") is reported once, on first"))
		fmt.Fprintln(a.Out, theme.Dim.Render("startup only."))
		fmt.Fprintln(a.Out)
