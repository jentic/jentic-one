...
// TestWireGracefulCancelSendsSIGTERM lives in run_cancel_unix_test.go (it uses
// POSIX signals + `sleep`); the Windows Kill()-on-cancel contract is covered by
// run_cancel_windows_test.go. wireGracefulCancel's cross-platform wiring
// (Cancel set, WaitDelay) is asserted in both.
