...
`METHOD URL` pair from the search hit (its `method` + `url`, e.g.
...
host-relative path. Pass `METHOD:URL` — the method + url from the
search/inspect step. The broker authenticates you and injects the stored
