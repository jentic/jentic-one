...
// skills/<name>/SKILL.md (repo root) plus its optional
// skills/<name>/references/*.md files; do not hand-edit them. `go:embed` cannot
...
// serves them at `GET /skills/<name>.md` (#651) and
// `GET /skills/<name>/references/<file>` (#1333). A single-file string/[]byte
// target can only hold one file, so the multi-skill set requires an embed.FS.
// The second embed pattern picks up the per-skill reference files; BundledNames
// skips directories, so the shipped *name* set is unaffected by the subdirs.
//
//go:generate sh -c "cd ../../.. && make skills"
//go:embed content/*.md content/*/references/*.md
...
// skill ships none. Like BundledNames, the embed is the single source of
// truth — there is no hand-maintained list.
func BundledReferences(name string) []string {
	entries, err := fs.ReadDir(bundledContent, bundledDir+"/"+name+"/references")
	if err != nil {
		// Most skills ship no references; a missing dir is the normal case.
		return nil
	}
...
// file — the same bytes the backend serves at
// `GET /skills/<name>/references/<file>` (drift-pinned like the skill bodies).
// References are always emitted verbatim: no frontmatter, no BaseURL
// interpolation, no managed block.
func RawBundledReference(name, file string) ([]byte, error) {
	data, err := bundledContent.ReadFile(bundledDir + "/" + name + "/references/" + file)
	if err != nil {
		return nil, fmt.Errorf("read bundled reference %q/%q: %w", name, file, err)
	}
	return data, nil
}
