...
var configWriterAllowlist = map[string]bool{
	...
	"internal/mcpcfg/writers.go":          true, // third-party MCP configs (~/.cursor/mcp.json, claude_desktop_config.json, ~/.codex/config.toml) — foreign runtimes' files, NOT jentic config.yaml; merge is idempotent and mode-preserving (2-E3)
