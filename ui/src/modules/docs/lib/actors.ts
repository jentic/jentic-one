 ...
 *      selected actor (`user` | `agent` | `service_account`);
...
/** The three actor identities the platform issues tokens for. */
export const ACTOR_TYPES = ['user', 'agent', 'service_account'] as const;
...
export const CALLER_GROUP_BLURB: Record<CallerGroup, string> = {
	agent: 'Typically called by an agent or service account.',
...
export function actorCounts(payload: ReferencePayload): Record<ActorType, number> {
	const counts = { user: 0, agent: 0, service_account: 0 } as Record<ActorType, number>;
