...
/** The actor identities the platform issues tokens for. */
...
export const CALLER_GROUP_BLURB: Record<CallerGroup, string> = {
	agent: 'Typically called by an agent.',
...
export function actorCounts(payload: ReferencePayload): Record<ActorType, number> {
	const counts = { user: 0, agent: 0 } as Record<ActorType, number>;
