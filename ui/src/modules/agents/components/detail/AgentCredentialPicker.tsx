/**
 * AgentCredentialPicker — searchable list of workspace credentials for the
 * agent-side "Bind credential" dialog (theme 5 phase 5a, transplanted from
 * the toolkit bind picker).
 *
 * Lists the credentials the caller already has (name + API/vendor + auth-type
 * badge), filters by name/vendor, hides the ones already bound to this agent,
 * and selects on click. Data comes from the agents service tier
 * (`useBindableCredentialsForAgent`), which reads the org-wide
 * `GET /credentials` surface through the shared API — no sibling
 ...
 * `type` (the projection deliberately doesn't carry the generated enum). */
const TYPE_LABELS: Record<string, string> = CREDENTIAL_TYPE_LABELS;

interface AgentCredentialPickerProps {
	/** Credential ids already bound to this agent — hidden from the list. */
	boundIds: Set<string>;
	/** Fired with the chosen credential (full row, so hosts can show its label). */
	onSelect: (credential: AgentBindableCredential) => void;
...
export function AgentCredentialPicker({
	boundIds,
	onSelect,
	pending,
	enabled = true,
	...
	const available = useMemo(() => {
		...
		return all.filter((c) => {
			...
			// typing what's on screen returns the row rather than only matching
			// the raw vendor/name machine identity.
		...
		<div className="space-y-3">
			...
			{!isLoading && !error && allBound && (
				<EmptyState
					...
					description="Every credential in your workspace is already bound to this agent."
				/>
			)}
...
function CredentialRow({
	...
	// credential_id so we never render blank.
	const title =
		cred.name ||
		apiRefDisplayName({
		...
		cred.provider ||
		cred.credential_id;
	// Muted technical subtitle: the machine identity when one exists, else a
	// guaranteed technical identifier (`credential_id`) so two identically-named
	// creds with no API identity stay disambiguable.
