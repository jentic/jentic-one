...
``token_endpoint_auth_method='none'`` is allowed only when
...
``admin/services/oauth_client_service.py``. A new writer (e.g. the anonymous DCR
...
_ALLOWED_CALLERS = frozenset(
    {
        ...
        # The anonymous DCR front door replicates the invariant by
...
def test_oauth_client_secret_hash_writers_are_confined_to_the_service() -> None:
    ...
    assert not violations, (
        ...
        "OAuthClientService, which enforces the none↔NULL-hash coupling. "
